...
const dbDao = new mysqlDao('localhost', '123456', 'contactDB')
...
api.get(`${URL}/contact/:id`, async (req, res) => {
    ...
    res.status(200).json({status: 'success', payload: contact});
});

//Get all contacts
api.get(`${URL}/contact`, async (req, res) => {
    const contacts = await dbDao.getContacts();
    res.status(200).json({status: 'success', payload: contacts});
});
